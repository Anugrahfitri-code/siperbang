            ...
            <div key={item.id} className="border border-slate-200 rounded p-4 flex flex-col justify-between hover:border-amber-300 hover:shadow-xs transition-all">
              <div>
                <div className="flex justify-between items-start mb-2">
                  <span className="text-[10px] font-mono font-bold text-slate-400 bg-slate-50 px-2 py-0.5 rounded uppercase">{item.code}</span>
                  {item.qty > 10 ? (
                    <span className="flex items-center gap-1 text-[10px] font-extrabold text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded border border-emerald-100">
                      <CheckCircle size={10} /> Aman
                    </span>
                  ) : item.qty > 0 ? (
                    <span className="flex items-center gap-1 text-[10px] font-extrabold text-amber-600 bg-amber-50 px-2 py-0.5 rounded border border-amber-100">
                      <AlertCircle size={10} /> Menipis
                    </span>
                  ) : (
                    <span className="flex items-center gap-1 text-[10px] font-extrabold text-rose-600 bg-rose-50 px-2 py-0.5 rounded border border-rose-100">
                      <AlertCircle size={10} /> Kosong
                    </span>
                  )}
                </div>
                <h3 className="text-sm font-extrabold text-slate-800 line-clamp-2">{item.name}</h3>
                <p className="text-[10px] text-slate-500 mt-1">Update: {item.lastUpdated}</p>
              </div>
              <div className="mt-4 pt-3 border-t border-slate-100 flex items-center justify-between">
                <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">Ketersediaan</span>
