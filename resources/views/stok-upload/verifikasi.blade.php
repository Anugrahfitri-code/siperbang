                        ...
                        @foreach($batch->details as $index => $row)
                            <tr class="hover:bg-slate-50/50 transition-colors {{ $row->status_validation === 'Perlu Perbaikan' ? 'bg-rose-50/5' : '' }}">
                                ...
                                <td class="px-4 py-3.5">
                                    ...
                                    <div class="text-[10px] text-slate-400 font-medium mt-0.5">Sheet: {{ $row->sheet_name }} • Supplier: {{ $row->supplier }}</div>
                                    @if($row->notes_error)
                                        <div class="text-[10px] text-rose-500 mt-1 font-semibold flex items-center gap-0.5">
                                ...
                                <!-- Original Excel Code -->
                                <td class="px-4 py-3.5 font-mono text-[11px] {{ $row->kode_persediaan_excel ? 'text-indigo-600 font-semibold' : 'text-slate-400' }}">
                                    {{ $row->kode_persediaan_excel ?? '[KOSONG]' }}
                                </td>

                                <!-- Suggested System Code -->
                                <td class="px-4 py-3.5 font-mono text-[11px] text-indigo-700 font-extrabold bg-indigo-50/20">
