    ...
    <div class="bg-white border border-slate-200 rounded-xl p-5 shadow-xs flex flex-col sm:flex-row justify-between items-center gap-4">
        ...
        <div class="flex gap-2 w-full sm:w-auto">
            @if($batch->status !== 'Selesai')
                <a href="{{ route('stok-upload.verifikasi.index', $batch->id) }}" class="flex-1 sm:flex-initial text-center px-4 py-2.5 rounded-lg border border-indigo-600 text-xs font-bold text-indigo-700 bg-white hover:bg-indigo-50 shadow-xs transition-colors">
                    Verifikasi Kode Persediaan
                </a>
                
                @if($batch->status !== 'Perlu Perbaikan')
                    <form action="{{ route('stok-upload.finalisasi', $batch->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin memfinalisasi data stok yang disetujui ke Master Barang?')" class="flex-1 sm:flex-initial">
                        @csrf
                        <button type="submit" class="w-full px-5 py-2.5 rounded-lg text-xs font-bold text-white bg-indigo-600 hover:bg-indigo-700 shadow-sm transition-colors">
                            Finalisasi Stok
                        </button>
                    </form>
                @else
                    <button class="flex-1 sm:flex-initial px-5 py-2.5 rounded-lg text-xs font-bold text-slate-400 bg-slate-100 border border-slate-200 cursor-not-allowed" disabled title="Perbaiki data error di halaman Verifikasi terlebih dahulu">
                        Finalisasi Stok (Terkunci)
                    </button>
                @endif
            @else
                <span class="text-emerald-700 font-bold text-xs flex items-center gap-1 bg-emerald-50 border border-emerald-150 px-4 py-2 rounded-lg">
