...
export default function App() {
  ...
  useEffect(() => {
    const savedRequests = localStorage.getItem("siperbang_requests");
    const savedStock = localStorage.getItem("siperbang_stock");
    const savedReceipts = localStorage.getItem("siperbang_receipts");
    const savedLogs = localStorage.getItem("siperbang_logs");
    const savedUsers = localStorage.getItem("siperbang_users");

    if (savedRequests) setRequests(JSON.parse(savedRequests));
    else setRequests(INITIAL_REQUESTS);

    if (savedStock) setStock(JSON.parse(savedStock));
    else setStock(INITIAL_STOCK);

    if (savedUsers) setUsers(JSON.parse(savedUsers));
    else {
      setUsers([
        { id: "u-1", name: "Iwan Setiawan", username: "iwan.s", role: UserRole.PETUGAS_PERSERDIAN, status: "Aktif" },
        { id: "u-2", name: "Budi Santoso", username: "budi.tu", role: UserRole.KETUA_TIM, section: "Tata Usaha", status: "Aktif" },
        { id: "u-3", name: "Admin Utama", username: "admin", role: UserRole.SUPERADMIN, status: "Aktif" }
      ]);
    }

    if (savedLogs) setLogs(JSON.parse(savedLogs));
    else {
      const initialLogs: LogType[] = [
        {
          id: "log-init-1",
        ...
        },
        {
          ...
          actor: "Iwan Setiawan (Petugas)",
          action: "Verifikasi Kode",
          details: "Mengunggah data persediaan awal dan memverifikasi kode persediaan.",
        },
      ];
      setLogs(initialLogs);
    }
    ...
    if (savedReceipts) setReceipts(JSON.parse(savedReceipts));
      ...
      // Add one pre-populated verified receipt so the metrics cards have nice initial numbers
      const prePopulatedReceipt: ReceiptData = {
        id: "rc-init",
        invoiceNo: "INV-C727-2026",
        storeName: "Koperasi Pegawai BBPSDM Makassar",
        date: "2026-07-06",
        isTaxed: true,
        taxRate: 11,
        subtotal: 1000000,
        ...
        bastName: "Koperasi Pegawai BBPSDM Makassar",
        bastDate: "2026-07-06",
      };
      setReceipts([prePopulatedReceipt]);
    }
  }, []);

  // Save changes to localStorage on states update
  useEffect(() => {
    if (requests.length > 0) localStorage.setItem("siperbang_requests", JSON.stringify(requests));
  }, [requests]);

  useEffect(() => {
    if (stock.length > 0) localStorage.setItem("siperbang_stock", JSON.stringify(stock));
  }, [stock]);

  useEffect(() => {
    if (receipts.length > 0) localStorage.setItem("siperbang_receipts", JSON.stringify(receipts));
  }, [receipts]);

  useEffect(() => {
    if (logs.length > 0) localStorage.setItem("siperbang_logs", JSON.stringify(logs));
  }, [logs]);

  useEffect(() => {
    if (users.length > 0) localStorage.setItem("siperbang_users", JSON.stringify(users));
  }, [users]);
